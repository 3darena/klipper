...
CharGlyphs = { 'degrees': bytearray(font6x12.VGA_FONT[0xf8]) }
...
class DisplayBase:
    ...
    def set_pixel(self, x, y):
        ...
        if not (0 <= x < 128 and 0 <= y < 64):
            return
        page = y & 31
        col  = (x // 8) + (0 if y < 32 else 16)
        mask   = 1 << (7 - (x % 8))             # bit7 = levý, bit0 = pravý
        self.graphics_framebuffers[page][col] |= mask
    def draw_glyph_pixels(self, x0, y0, halves):
        """
        Nakreslí 12×12 bitmapu (icon je bytes/bytearray délky==12)
        na pixelovou pozici (x0,y0).
        """
        # vykreslíme každou 6×12 polovinu
        for half_idx, rows in enumerate(halves):
            for ry, row_bits in enumerate(rows):
                for rx in range(CHAR_WIDTH):
                    # MSB vlevo (otočené vodorovně)
                    if (row_bits >> (CHAR_WIDTH - rx + 1)) & 1:
                        self.set_pixel(x0 + half_idx * CHAR_WIDTH + rx, y0 + ry + OFFSET)
    def draw_char_pixels(self, x0, y0, glyph):
        """
        Nakreslí 6×12 bitmapu (glyph je bytes/bytearray délky>=12) na pozici (x0,y0).
        """
        for ry in range(CHAR_HEIGHT):
            row_bits = ((glyph[ry] >> 2) | ((glyph[ry] << (CHAR_WIDTH-2)) & ((1<<CHAR_WIDTH)-1))) & ((1<<CHAR_WIDTH)-1)
            for rx in range(CHAR_WIDTH):
                 if (row_bits >> (CHAR_WIDTH - 1 - rx)) & 1:
                    self.set_pixel(x0 + rx, y0 + ry + OFFSET)
    def flush(self):
    ...
    def write_text(self, col, row, data):
#        self.set_pixel(0,0)
#        self.set_pixel(0,63)
#        self.set_pixel(127,0)
#        self.set_pixel(127,63)
#        for q in range(21):
#            for w in range(63):
#                self.set_pixel(q*6,w)
#            self.set_pixel(127-q,q)
        """
        Vykreslí řetězec data na textové pozici (col,row) pixelově.
        """
        if col < 0 or col >= 21 or not data:
            return
        ...
        for ch in data:
            code = ord(ch)
            if code < 32 or code >= len(VGA_FONT):
                code = 32
            glyph = VGA_FONT[code]
            self.draw_char_pixels(x0, y0, glyph)
            x0 += CHAR_WIDTH + 0
...
#        icon = self.icons.get(glyph_name)
        ...
        if halves:
            # počáteční pixelové souřadnice
            x0 = x * CHAR_WIDTH
            y0 = y * CHAR_HEIGHT
            self.draw_glyph_pixels(x0, y0, halves)
            return 2
    ...
    def get_dimensions(self):
        return (128 // CHAR_WIDTH, 64 // CHAR_HEIGHT)
#        return (21, 5)

# Display driver for stock ST7920 displays
class ST7920HD(DisplayBase):
